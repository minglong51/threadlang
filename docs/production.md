...
- Completed step outputs are checkpointed only after their outgoing edge has resolved; emit output is not a step checkpoint.
...
Maintainers publish packages through the gated procedure in
[`RELEASING.md`](../RELEASING.md); production operators do not need that
workflow.
